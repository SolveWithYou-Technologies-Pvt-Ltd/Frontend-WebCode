...
import { fetchEmployeesForAssignment, createClientProject } from "../../api/clientProjectApi";
...
const ClientProjectCreate = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        const propData = await fetchAdminProposalById(proposalId);
        ...
        setFormData(prev => ({
          ...
          clientName: propData.quote?.fullName || ""
        }));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await createClientProject({ ...formData, proposalId });
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-10">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" disabled={submitting} className="px-4 py-2 bg-teal-600 text-white rounded-lg text-[13px] font-bold flex items-center gap-2 disabled:opacity-70"><Save size={14}/> Create Project</button>
          </div>
        </form>
      </div>
    </div>
